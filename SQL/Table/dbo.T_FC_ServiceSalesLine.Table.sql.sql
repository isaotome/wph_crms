	...
	BEGIN
		...
		BEGIN TRY
			...
			FROM
				dbo.sysobjects
			WHERE
				id = object_id(N'T_FC_ServiceSalesLine')
			...
			BEGIN
				--テーブル作成
				CREATE TABLE [dbo].[T_FC_ServiceSalesLine](
				...
				--EUCDBのデータをWPH_DBに移行
				INSERT INTO
					[WPH_DB].[dbo].[T_FC_ServiceSalesLine]
					SELECT
						*
					FROM
						[EUCDB].[dbo].[T_FC_ServiceSalesLine]
				...
				INSERT INTO [dbo].[DB_ReleaseHistory]
				 ...
				 VALUES
					   (NEWID()
					   ,'3725'--チケット番号
					   ,'20170313_#3725_サブシステム移行（整備履歴）/07_Create_Table_T_FC_ServiceSalesHeader.sql'
