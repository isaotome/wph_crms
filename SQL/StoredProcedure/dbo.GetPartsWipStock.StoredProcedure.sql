...
/****** Object:  StoredProcedure [dbo].[GetPartsWipStock]    Script Date: 2018/12/11 13:27:04 ******/
SET ANSI_NULLS ON
GO
...
-- 2018/10/25 yano #3952 仕掛在庫検索 未引当の部品も仕掛中として表示される
-- 2018/10/01 yano #3942 仕掛在庫検索　部品が表示されない
		...
		BEGIN
			...
			*/
			...
			SET @PARAM = '@ServiceType NVARCHAR(3), @PartsNumber NVARCHAR(25), @PartsNameJp NVARCHAR(50)'
				...
				SET @SQL = @SQL +',		ISNULL(sl.[ProvisionQuantity], 0) AS Quantity' + @CRLF							--数量
				...
				SET @SQL = @SQL +'				AND ISNULL(sl.ProvisionQuantity, 0) > 0' + @CRLF					--Add 2018/10/25 yano #3952
			...
			BEGIN
				...
				--SET @SQL = @SQL +'	AND (' + @CRLF													--Add 2016/07/14 arc yano	--Dell 2018/10/01 yano #3942
				--SET @SQL = @SQL +'			P.PartsNumber is NULL' + @CRLF							--Add 2016/07/14 arc yano	--Dell 2018/10/01 yano #3942
				--SET @SQL = @SQL +'			OR C2.SelectedGenuineType = P.GenuineType' + @CRLF		--Add 2016/07/14 arc yano	--Mod 2017/10/19 arc yano #3803
				--SET @SQL = @SQL +'		)' + @CRLF													--Dell 2018/10/01 yano #3942
				--並び替え
