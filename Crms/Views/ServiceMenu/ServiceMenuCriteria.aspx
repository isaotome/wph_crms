<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/MainPage.Master" Inherits="System.Web.Mvc.ViewPage<CrmsDao.PaginatedList<CrmsDao.ServiceMenu>>" %>
...
<%using (Html.BeginForm("Criteria", "ServiceMenu", new { id = 0 }, FormMethod.Post)){ %>
<%=Html.Hidden("id", "0") %>
...
<br />
<input type="button" value="新規作成" onclick="openModalAfterRefresh('/ServiceMenu/Entry')"/>
<br />
<br />
...
<table class="list">
    ...
    <%foreach (var serviceMenu in Model)
      { %>
    <tr>
        <td><a href="javascript:openModalAfterRefresh('/ServiceMenu/Entry/' + '<%=serviceMenu.ServiceMenuCode%>')">詳細</a></td>
        <td><%=Html.Encode(serviceMenu.ServiceMenuCode)%></td>
        <td><%=Html.Encode(serviceMenu.ServiceMenuName)%></td>
        <td><%=Html.Encode(serviceMenu.DelFlagName)%></td>
    </tr>
    <%} %>
</table>
<br />
