...
<%-- ----------------------------------------------------------------
   機能:車両管理表取込
　 作成日：2018/08/28 yano #3922 車両管理表(タマ表) 機能改善② 新規作成
------------------------------------------------------------------ --%>

    <%using (Html.BeginForm("ImportDialogForCarStock", "CarStock", FormMethod.Post, new { enctype = "multipart/form-data" }))
      { %>
    <%=Html.Hidden("ErrFlag", ViewData["ErrFlag"]) %>
    <%=Html.Hidden("RequestFlag", ViewData["RequestFlag"]) %><%//押下ボタン判定(1:読み込み / 2:取り込み )%>
    <%=Html.Hidden("TargetMonth", ViewData["TargetMonth"]) %>
<table class="command">
   <tr>
