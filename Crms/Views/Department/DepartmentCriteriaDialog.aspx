...
<%using (Html.BeginForm("CriteriaDialog", "Department", new { id = 0 }, FormMethod.Post)) { %>
<%=Html.Hidden("id", "0") %>
<% //2014/09/08 arc yano IPO対応その２　月締め処理状況画面／それ以外の画面により、ポスト先を変更する。%>
<%=Html.Hidden("FormAction", ViewData["FormAction"]) %>

<% //2017/06/23 arc yano #3762 車両在庫棚卸機能追加%>
<%=Html.Hidden("CarInventoryFlag", ViewData["CarInventoryFlag"]) %>
<%=Html.Hidden("PartsInventoryFlag", ViewData["PartsInventoryFlag"]) %>
