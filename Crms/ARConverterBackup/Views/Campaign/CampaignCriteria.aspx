<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/MainPage.Master" Inherits="System.Web.Mvc.ViewPage<CrmsDao.PaginatedList<CrmsDao.Campaign>>" %>
...
<%using (Html.BeginForm("Criteria", "Campaign", new { id = 0 }, FormMethod.Post)){ %>
<%=Html.Hidden("id", "0") %>
...
<%} %>
<br />
<input type="button" value="新規作成" onclick="openModalAfterRefresh('/Campaign/Entry')" />
<br />
<br />
...
<table class="list">
    ...
    <%foreach (var campaign in Model)
      { %>
    <tr>
        <td><a href="javascript:void(0);" onclick="openModalAfterRefresh('/Campaign/Entry/' + '<%=campaign.CampaignCode%>');return false;">詳細</a></td>
        <td><%=Html.Encode(campaign.CampaignName)%></td>
